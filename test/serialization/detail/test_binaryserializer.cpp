...
void TestBinarySerializer()
{
	...
	
	{
		...
		
		{
			...
			NNAssertEquals(d.get<SerializedArray>("emptyArray").size(), 0, "BinarySerializer failed!");
			NNAssertEquals(d.get<SerializedObject>("emptyObject").size(), 0, "BinarySerializer failed!");
			
			NNAssertEquals(d.get<Serialized *>("nested")->get<bool>("indented"), true, "BinarySerializer failed!");
			NNAssertEquals(d.get<Serialized *>("nested")->get<std::string>("powerLevel"), "> 9000", "BinarySerializer failed!");
		}
	}
	...
	
	{
		...
		Sequential<NN_REAL_T> *deserialized = BinarySerializer::read(ss).as<Sequential<NN_REAL_T> *>();
