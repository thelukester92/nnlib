...
void TestNLL()
{
	...
	NNHardAssert(critic.inGrad().addM(grd, -1).square().sum() < 1e-12, "NLL<NN_REAL_T>::backward with no average failed!");
	
	critic.average(true);
	critic.backward(inp, tgt);
	NNHardAssert(critic.inGrad().addM(grd, -1.0 / inp.size()).square().sum() < 1e-12, "NLL<NN_REAL_T>::backward with average failed!");
}
