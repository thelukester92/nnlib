...
NNTestClassImpl(Tensor)
{
    ...
    NNTestMethod(rand)
    {
        NNTestParams()
        {
            ...
            Tensor<T> t = Tensor<T>(1000).rand();
            T expectedMean = 0, expectedVariance = 4.0 / 12.0;
            T mean = 0, variance = 0, min = t(0), max = t(0);
            ...
            variance /= t.size();
            NNTestAlmostEquals(mean, expectedMean, 0.1);
            NNTestAlmostEquals(variance, expectedVariance, 0.1);
            NNTestGreaterThanOrEquals(min, -1.0);
            NNTestLessThanOrEquals(max, 1.0);
        }

        NNTestParams(const T &, const T &)
        {
            RandomEngine::sharedEngine().seed(0);
            Tensor<T> t = Tensor<T>(1000).rand(3.14 - 1.5, 3.14 + 1.5);
            T expectedMean = 3.14, expectedVariance = 9.0 / 12.0;
            T mean = 0, variance = 0, min = t(0), max = t(0);
            for(size_t i = 0; i < t.size(); ++i)
            {
                mean += t(i);
            ...
            NNTestAlmostEquals(mean, expectedMean, 0.1);
            NNTestAlmostEquals(variance, expectedVariance, 0.1);
            NNTestGreaterThanOrEquals(min, 3.14 - 1.5);
            NNTestLessThanOrEquals(max, 3.14 + 1.5);
        }
    }
    ...
    NNTestMethod(randn)
    {
        ...
        {
            RandomEngine::sharedEngine().seed(0);
            Tensor<T> t = Tensor<T>(1000).randn();
            T expectedMean = 0, expectedVariance = 1;
            T mean = 0, variance = 0;
            for(size_t i = 0; i < t.size(); ++i)
                mean += t(i);
            mean /= t.size();
        ...
        {
            RandomEngine::sharedEngine().seed(0);
            Tensor<T> t = Tensor<T>(1000).randn(3.14, 3.0 / sqrt(12.0));
            T expectedMean = 3.14, expectedVariance = 9.0 / 12.0;
            T mean = 0, variance = 0;
            for(size_t i = 0; i < t.size(); ++i)
                mean += t(i);
        ...
        NNTestParams(const T &, const T &, const T &)
        {
            RandomEngine::sharedEngine().seed(0);
            Tensor<T> t = Tensor<T>(1000).randn(3.14, 3.0 / sqrt(12.0), 3);
            T expectedMean = 3.14, expectedVariance = 9.0 / 12.0;
            T mean = 0, variance = 0, min = t(0), max = t(0);
            for(size_t i = 0; i < t.size(); ++i)
            {
            ...
            NNTestAlmostEquals(mean, expectedMean, 0.1);
            NNTestAlmostEquals(variance, expectedVariance, 0.1);
            NNTestGreaterThanOrEquals(min, 3.14 - 3);
            NNTestLessThanOrEquals(max, 3.14 + 3);
        }
    }
    ...
    NNTestMethod(bernoulli)
    {
        ...
        {
            RandomEngine::sharedEngine().seed(0);
            Tensor<T> t = Tensor<T>(1000).bernoulli();
            size_t ones = 0;
            for(size_t i = 0; i < t.size(); ++i)
                if(t(i) > 0.5)
                    ++ones;
            NNTestAlmostEquals(ones, t.size() * 0.5, 25);
        }

        NNTestParams(const T &)
        {
            RandomEngine::sharedEngine().seed(0);
            Tensor<T> t = Tensor<T>(1000).bernoulli(0.75);
            size_t ones = 0;
            for(size_t i = 0; i < t.size(); ++i)
                if(t(i) > 0.5)
                    ++ones;
            NNTestAlmostEquals(ones, t.size() * 0.75, 25);
        }
    }
