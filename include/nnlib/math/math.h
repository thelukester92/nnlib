...
public:
	...
	static void mAdd_mm(
		const T *A, size_t ra, size_t ca, size_t lda,
		const T *B, size_t rb, size_t cb, size_t ldb,
		T *C, size_t rc, size_t cc, size_t ldc,
		T alpha = 1, T beta = 0
	)
	{
		NNAssert(ra == rc && cb == cc && ca == rb, "Incompatible operands to mm!");
	...
	{
		NNAssert(ca == rc && cb == cc && ra == rb, "Incompatible operands to mtm!");
		for(size_t i = 0; i < rc; ++i)
		{
			for(size_t j = 0; j < cc; ++j)
			{
	...
	{
		NNAssert(ra == rc && rb == cc && ca == cb, "Incompatible operands to mmt!");
		for(size_t i = 0; i < rc; ++i)
		{
			for(size_t j = 0; j < cc; ++j)
			{
